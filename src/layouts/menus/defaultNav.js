...
export const DefaultNavHeader = ({ title }) =>{
    ...
    const RenderTitle = ({ title }) => {
        return (<View style={styles.iconWithTitle}> 
            <View>
            ...
            <View style={{ marginLeft: 30}}>
                <Text 
                    ...
                    style={[styles.generalTextstyle, styles.textColor, styles.routeTitleText]}>{title}</Text>
            </View> 
        </View>)
    }
    ...
    return(
        <View style={styles.defaultNavContainer}>
            <View style={{flex: 2}}>
                <TouchableOpacity onPress={() => { navigation.canGoBack() ? navigation.goBack(): navigation.navigate(routes.WELCOME) }}>
                 {title ? <RenderTitle title={title}/>: <AppIcon name="arrow-left"  size={28}/>}
                </TouchableOpacity>
            </View>
